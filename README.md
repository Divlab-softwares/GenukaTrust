# GenukaTrust - collecteur d'avis intelligent

<p align="center"> <img src="https://img.shields.io/badge/Status-Hackathon%20Project-blue?style=for-the-badge"/> <img src="https://img.shields.io/badge/Platform-Genuka-orange?style=for-the-badge"/> <img src="https://img.shields.io/badge/Powered%20By-CIA CodeStorm%20%F0%9F%A4%96-green?style=for-the-badge"/> </p> 
<p align="center"> <img src="https://img.shields.io/github/languages/top/?style=flat-square&logo=code&label=Tech Stack"/></p>
<p align="center"> <img src="https://via.placeholder.com/900x400/162447/FFFFFF?text=GenukaTrust+-+Smart+Reviews+Dashboard" width="90%"/> </p>

## 🗂 1.  ___Catégories du projet___

- Avis & Reviews 
- KPI & Performance 
- Marketing Automation 
- Analytics 
- IA (Analyse de sentiment) 
- Notifications intelligentes

## 🚀 2. ___Description brève du projet___

Genuka Trust est une application intelligente connectée à la plateforme Genuka, conçue pour automatiser entièrement la collecte, la gestion et la valorisation des avis clients.

### 💡 ___Comment ça marche ?___

   - Lorsqu’une commande est livrée(statut terminé), le client reçoit automatiquement un SMS/WhatsApp pour noter son expérience ( note de 1 à 5 étoiles et un commentaire)  ⭐

   - Son avis est analysé par une IA de sentiment 🤖

   - Les avis positifs sont intégrés dans : un widget d’avis pour le site Genuka du marchand, des images prêtes pour réseaux sociaux, des badges de confiance dynamiques.

### 👉 Objectif : aider les PME à transformer leurs avis clients en véritable levier de confiance, augmenter la preuve sociale, booster la conversion, et créer de la confiance automatiquement.

<p align="center"> <img src="https://via.placeholder.com/900x350/EAEAEA/333333?text=Widget+Reviews+Preview" width="90%"> </p>

## ❗ 3.  ___Problème que GenukaTrust cherche à résoudre___

  ### La majorité des PME :  
    ❌ Ne demandent pas assez d’avis  
    ❌ Perdent les feedbacks clients  
    ❌ N’ont pas d’outils de mise en valeur  
    ❌ N’analysent pas la satisfaction client  

  ### Résultat :  
    ➡ Faible crédibilité  
    ➡ Conversions limitées  
    ➡ Mauvaises décisions business  

  ### ✅ La Solution apportée par GenukaTrust

    ✔ Collecte d’avis automatique  
    ✔ Analyse IA (positif / neutre / négatif)  
    ✔ Tableau de bord clair & actionnable  
    ✔ Badges de confiance pour rassurer les visiteurs  
    ✔ Widget d’avis moderne pour les boutiques Genuka  
    ✔ Export & partage automatique sur réseaux sociaux  

<p align="center"> <img src="https://via.placeholder.com/900x350/1e1e2f/FFFFFF?text=Dashboard+IA+Sentiment+Analysis" width="90%"/> </p>

## 🛠 4.  ___Technologies utilisées___

| Domaine             | Technologies                                 |
| ------------------- | -------------------------------------------- |
| **Frontend**        | React, Next.js, TailwindCSS                  |
| **Backend**         | Node.js, Laravel ou NestJS                   |
| **IA**              | Analyse des Sentiments (NLP), modèles légers |
| **APIs**            | API Genuka, WhatsApp Cloud API, SMS Gateway  |
| **Base de données** | PostgreSQL                                   |

Badges techniques
<p align="left"> <img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB"/> <img src="https://img.shields.io/badge/Next.js-black?style=for-the-badge&logo=next.js"/> <img src="https://img.shields.io/badge/TailwindCSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white"/> <img src="https://img.shields.io/badge/Node.js-6DA55F?style=for-the-badge&logo=node.js&logoColor=white"/> <img src="https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white"/> </p>

## 5. ___Aperçus & Maquettes___

🧩 Widget d’avis
<p align="center"><img src="https://img.widget.png/700x300/FFFFFF/333333?text=Avis+Widget+UI" width="80%"></p>
📈 Dashboard IA 
<p align="center"><img src="https://via.placeholder.com/700x300/182848/FFFFFF?text=Dashboard+Trust+Analytics" width="80%"></p>
⭐ Badges de confiance
<p align="center"><img src="https://via.placeholder.com/600x200/EDEDED/333333?text=Trusted+Seller+Badges" width="70%"></p>

## 🙌 6. Équipe & Contributions

  ✨ UX/UI  
  ✨ Développement Frontend  
  ✨ Backend & API  
  ✨ Analyse IA  
  ✨ Intégration Genuka  
